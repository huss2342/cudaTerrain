...
__global__ void visualizeTerrainWithHeight(int* terrain, float* heightMap, unsigned char* image, int width, int height) {
    ...
    if (x < width && y < height) {
        ...
        int r = min(255, (int)(terrainInfo->color.r * shadingIntensity));
        int g = min(255, (int)(terrainInfo->color.g * shadingIntensity));
        int b = min(255, (int)(terrainInfo->color.b * shadingIntensity));
