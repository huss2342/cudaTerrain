...
// Define terrain data in the same format as the GPU version
...
// Static array for CPU implementation (equivalent to GPU __constant__ memory)
TerrainType TerrainTypes::TERRAINS[31];

// Function to initialize terrain types
void TerrainTypes::initializeTerrainTypes() {
    // Copy the static data to the TERRAINS array
    memcpy(TERRAINS, terrainData, sizeof(TerrainType) * 31);
}

// Implementation of getTerrainById similar to GPU version
const TerrainType* TerrainTypes::getTerrainById(int id) {
